...
st.markdown("""
    ...
    .reason-box { background-color: #131722; padding: 20px; border-radius: 10px; border-left: 5px solid #2962ff; color: white; }
    </style>
...
if pcr_input > 1.3:
    pcr_reason = "PCR kafi high hai (>1.3), iska matlab Heavy Put Writing (Strong Support) ho rahi hai. Market niche girna mushkil hai."
elif pcr_input < 0.7:
    pcr_reason = "PCR kafi low hai (<0.7), market Oversold zone me hai ya Heavy Call Writing (Strong Resistance) hai. Upper side par pressure rahega."
else:
    pcr_reason = "PCR neutral range me hai, market data-driven solid breakout ka wait kar raha hai."

if iv_input * 100 > 18:
    iv_reason = "IV (Volatility) badhi hui hai, jiski wajah se premiums me bade sharp jumps dekhne ko milenge (High Risk Option Buyers Market)."
else:
...
<div class="reason-box">
    ...
    <p><b>3. Mathematical Trigger:</b> Agar Nifty <b>₹{spot_price}</b> se <b>₹{future_spot}</b> jata hai, toh niche di gayi Option Chain ke mutabik net change calculate kiya gaya hai.</p>
</div>
""", unsafe_allow_html=True)
